...
void cFCgraphics::SetDivideOptions(int nl, int nc){
    nlines = nl;
    ncolus = nc;
}
...
void cFCgraphics::Draw(string sopt) {
  ...
  if (sopt == "all") {
    ...
    if (nlines && ncolus || count<3) {
      fCanvas->Divide(ncolus,nlines);
    } else {
...
void cFCgraphics::DrawPad(string PadName) {
  ...
  itMpad = Mpad.find(PadName);
  //printf("ncanvas %d, mycanvasn %d\n", Ncanvas, MyCanvasNumber);
  if (itMpad != Mpad.end()) {
    fCanvas->cd();
